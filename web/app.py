...
def save_normalized_image(path, data, name):
	...
	image.save(path)
	new_path = 'data/{}_output'.format(name)
	process = subprocess.Popen("python3 ../WarpGAN/test.py ../WarpGAN/pretrained/warpgan_pretrained {} {} --num_styles 1".format(path, new_path), shell=True, stdout=subprocess.PIPE)
	process.wait()
	new_path += '_0.jpg'
...
def post():
	...
	path = ''
	try:
		(okay, path) = save_normalized_image(target, flask.request.data, sha1sum)
		if okay:
  ...
  function file_select_handler(to_upload) {
      ...
      xhr.onreadystatechange = function(e1) {
          if (this.readyState == 4)  {
              if (this.status == 200)
                  var text = '<img src="'+this.responseText+'" />';
              else
